...
NODES=$(kubectl exec -it -n "$NAME" deploy/switchlynode -c switchlynode -- curl -s http://localhost:1317/switchly/nodes)
...
VAULTS=$(kubectl exec -it -n "$NAME" deploy/switchlynode -c switchlynode -- curl -s http://localhost:1317/switchly/vaults/yggdrasil)
...
UNSPENT=$(kubectl exec -it -n "$NAME" deploy/bifrost -c bifrost -- sh -c "curl --user switchly:password -s --data-binary \"$LIST_UNSPENT\" http://\$${CHAIN}_HOST")
CHAIN_BALANCE=$(echo "$UNSPENT" | jq '([.result[]|.amount]|add//0)*1e8')

echo "=> Yggdrasil address: $YGG_ADDR"
echo "=> Switchly balance: $YGG_BALANCE"
...
kubectl exec -it -n "$NAME" deploy/bifrost -c bifrost -- sh -c "curl --user switchly:password -s --data-binary \"$IMPORT_ADDRESS\" http://\$${CHAIN}_HOST"
